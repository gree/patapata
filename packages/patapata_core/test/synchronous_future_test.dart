...
void main() {
  group('SynchronousErrorableFuture', () {
    ...
    test('then() should call onError with error and stackTrace', () async {
      ...
      final tFuture = SynchronousErrorableFuture<int>.error(
        'error message',
        tStackTrace,
      );
    ...
    test('catchError() should not modify value if no error', () async {
      final tFuture = SynchronousErrorableFuture<int>(
        42,
      ).catchError((error) => 99);
      await expectLater(tFuture, completion(42));
    });
    ...
    test('catchError() should handle error when test returns true', () async {
      ...
      final tResult = tFuture.catchError((error) => 99, test: (error) => true);

      await expectLater(tResult, completion(99));
    });

    test(
      'catchError() should not handle error when test returns false',
      () async {
        final tFuture = SynchronousErrorableFuture<int>.error('error message');
    ...
    test('timeout() should not trigger onTimeout', () async {
      ...
      final tFuture = SynchronousErrorableFuture<int>(42).timeout(
        Duration.zero,
        onTimeout: () {
    ...
    test(
      'wait() should resolve mixed synchronous and asynchronous futures',
      () async {
        final tFutures = [
          SynchronousErrorableFuture<int>(1),
    ...
    test(
      'wait() should execute whenComplete after all futures resolve',
      () async {
        ...
        final tResult = SynchronousErrorableFuture.wait(tFutures).whenComplete(
          () {
            tCompleted = true;
          },
        );
    ...
    test(
      'whenComplete() should replace future error if action throws an error',
      () async {
        final tFuture = SynchronousErrorableFuture<int>(42).whenComplete(() {
          throw 'new error';
        });

...
void main() {
  ...
  test('You can access correct data from a variable', () {
    expect(_A().intVariable.unsafeValue, equals(_kIntValue));
  });

  test('You can access correct data from a variable when a Map', () {
    expect(_A().mapVariable.unsafeValue, equals(_kMapValue));
  });
  ...
  test(
    'You can begin a batch when another batch is active with the same key',
    () {
      final tA = _A();
      tA.begin(tA.keyA);
  ...
  test(
    'When you commit a batch with no notify, listeners do not get notified',
    () {
      final tA = _A();
  ...
  test('You can lock for a batch', () async {
    final tA = _A();
    expect(await tA.lock((batch) => batch.commit()), equals(true));
  });
  ...
  test(
    ...
    () async {
      final tA = _A();
      expect(await tA.lock((batch) {}), equals(true));
    },
  );

  test(
    'You can lock for a batch and will return false on batch cancel',
    () async {
      final tA = _A();
      expect(await tA.lock((batch) => batch.cancel()), equals(false));
    },
  );
  ...
  test(
    ...
    () async {
      ...
      try {
        ...
        expect(
          error.toString(),
          startsWith('ConflictException: _A(BatchLockKey'),
        );
      }
  ...
  test(
    'If a lock throws, other awaiting locks will execute normally',
    () async {
      final tA = _A();
    ...
    () async {
      ...
      for (var i = 0; i < tInputs.length; i++) {
        ...
        final tResultA = tA.lock(
          ...
          override: tInput[0],
          overridable: tInput[1],
        );

        final tResultB = tA.lock(
          (batch) async {
  ...
  test(
    'An overriden lock has a chance to revert its changes with onOverride.',
    () async {
      final tA = _A();
  ...
  test('A variable can be created unset', () {
    final tA = _A();
    expect(() => tA.customUnsetVariable.unsafeValue, throwsAssertionError);
    expect(tA.customUnsetVariable.toString(), equals('_CustomType:NOT SET'));
  });
  ...
  test('A variable can be created unset, but set later and accessed', () {
    ...
    tA.begin()
      ...
      ..commit();
    expect(tA.customUnsetVariable.unsafeValue, isNotNull);
    expect(tA.customUnsetVariable.toString(), equals('_CustomType:CustomType'));
  });
  ...
  test(
    'Listeners are notified when an unset variable is set to the same value',
    () {
      final tA = _A();
  ...
  test('A model can be used', () async {
    ...
    final tSubscription = tA.use(
      expectAsync1((_A model) => model.ii, count: 3),
    );
  ...
  test(
    'A model can be used but the callback is called asynchronously',
    () async {
      final tA = _A();

      final tSubscription = tA.use(
        expectAsync1((_A model) => model.ii, count: 2),
      );
...
class _A extends ProviderModel<_A> {
  ...
  final keyA = ProviderLockKey(
    'keyA',
    conflictReason: 'Custom Conflict Reason',
  );

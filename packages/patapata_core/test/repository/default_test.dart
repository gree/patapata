...
class TestApp {
  ...
  Widget createMyApp<T extends RepositoryModel<T, int>>(
    ...
    return MaterialApp(
      home: TestProvider<T>(repository: repository),
    );
  }
}
...
class TestHasNotProviderBuilderRepository
  ...
  Map<Type, Future<HasNotBuilderModel?> Function(int id)>
      get singleSetFetchers =>
          <Type, Future<HasNotBuilderModel?> Function(int id)>{
            HasNotBuilderModel: (id) {
              return Future.value(
                HasNotBuilderModel.init(id, id),
              );
            },
          };
}
...
void main() {
  ...
  group('Test default repository', () {
    for (final flag in tTestInTreeRepository) {
      testWidgets('Test single set and fetch. repository in tree = $flag',
        ...
        final tRepository = testApp.asRepository<TestModel>() ??
      ...
      testWidgets('Test multi set and fetch. repository in tree = $flag',
          monitoringTimers((WidgetTester tester) async {
        await tester.pumpWidget(
          testApp.createMyApp(
            testData,
        ...
        // test update cache
        testData.pattern = TestPattern.type2;
        tRepository.fetchMany([1, 2, 3], TestModel,
            fetchPolicy: RepositoryFetchPolicy
                .noCache).then((data) => tModelList = data);
        await tester.pumpAndSettle();
      ...
      testWidgets('Test single set and store. repository in tree = $flag',
          monitoringTimers((WidgetTester tester) async {
        await tester.pumpWidget(
          testApp.createMyApp(
            testData,
      ...
      testWidgets('Test multi set and store. repository in tree = $flag',
          monitoringTimers((WidgetTester tester) async {
        await tester.pumpWidget(
          testApp.createMyApp(
            testData,
        ...
        fCompare(List<int> ids, bool checkAssertion) async {
          ...
          final tOtherRecords = Map.fromIterables(
              ids,
              ids
                  .map((e) => (42 + e, 0.42 + e, 'id / $e test store'))
                  .toList());
          tProvider.change<TestModel>(ids);
          await tester.pumpAndSettle();
          await tRepository.storeMany(
    ...
    testWidgets('Test overflow cache size in repository.',
        monitoringTimers((WidgetTester tester) async {
      await tester.pumpWidget(
        testApp.createMyApp(
          testData,
      ...
      final tRepository = findContext(tester).read<TestRepository<TestModel>>();
    ...
    testWidgets('Other test.', monitoringTimers((WidgetTester tester) async {
      await tester.pumpWidget(
        testApp.createMyApp(
          testData,
      ...
      await tester.runAsync(() async {
        ...
        try {
          await tRepository.fetch(1, Filter1,
              fetchPolicy: RepositoryFetchPolicy.noCache);
        } catch (e) {
    ...
    testWidgets('Test without inserting Provider into the Widget Tree',
        monitoringTimers((WidgetTester tester) async {
      await tester.pumpWidget(
        testApp.createMyApp(
          testData,
